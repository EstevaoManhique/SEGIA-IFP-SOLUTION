...
export default {
  ...
  data: () => ({
    ...
    opcoespauta: [
      { text: "Geral", value: -1 },
      { text: "Excluidos", value: "Excluido" },
      { text: "Admitidos", value: "Admitido" },
    ],
    opcoescurso: [
      { text: "Todos", value: -1 },
      { text: "12.a + 1", value: 1 },
  ...
  methods: {
    ...mapActions([
      ...
      "getDistricts",
      "getAvaliacoesBySchool",
    ]),
    initialize() {},
    getColor(state) {
      let x = state.includes("Admitido") ? "green" : "red";
      return x;
    },
    filterProvinces() {
    ...
    filterCourses(idSchool) {
      this.getAvaliacoesBySchool(idSchool);
    ...
    filterByCourse(courseId) {
      ...
      if (courseId == -1) {
        this.avalcs = this.avaliacoes;
      } else {
        this.avalcs = this.avaliacoes.filter((a) => {
          if (a.course_id == courseId) {
            return a;
          }
        });
      }
      this.geral = this.avalcs;
