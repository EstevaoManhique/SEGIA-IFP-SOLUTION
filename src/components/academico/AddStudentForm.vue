          ...
          <v-row>
            ...
            <v-col cols="12" md="4" sm="6">
              <v-select
                ...
                filled
                mandatory
                v-model="student.person.nationality_id"
              ></v-select>
            </v-col>
          ...
          <v-row>
            ...
            <v-col cols="12" sm="6" md="4">
              <v-select
                ...
                filled
                mandatory
                v-model="student.person.district"
              ></v-select>
            </v-col>
            <v-col cols="12" sm="6" md="4">
              <v-text-field
...
export default {
  ...
  methods: {
    ...mapActions(['getProvinces', 'addStudent']),
    setProvinceName() {
      this.$api
        .get('province')
        .then((data) => {
          this.provinces = data.data.sort((a, b) =>
            a.name.localeCompare(b.name)
          );
          this.provinces_name = this.provinces.map((n) => {
    ...
    changProv() {
      this.districts_name = this.provinces
        .filter((n) => n.id == this.student.person.province)[0]
        .districts.map((n) => {
          return { text: n.name, value: n.id };
        });
    },
  ...
  mounted() {
    ...
    this.$api.get('config/nationality').then((data) => {
      ...
      this.nat = this.nationalities.map((n) => {
        return { text: n.description, value: n.id };
      });
    });
  },
  computed: {
